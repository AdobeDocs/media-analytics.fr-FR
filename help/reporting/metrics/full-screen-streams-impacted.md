...
Le serveur principal du média définit l’indicateur de `isSet` dans `mediaReporting.states[]` pour que l’entrée `fullscreen` `true` la première fois qu’un événement de `media.statesUpdate` avec `fullscreen` dans `statesStart` est reçu. La mesure est signalée lors de l’appel de fermeture.

| Système de reporting | Source |
| --- | --- |
| Adobe Analytics | Collecté automatiquement à partir des `a.media.states.fullscreen.set` de données contextuelles lorsque le [[!UICONTROL suivi de l’état du lecteur]](/help/reporting/media-reports-enable.md) est activé. |
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/fr/docs/experience-platform/xdm/data-types/media-reporting-details) l’entrée où `name = "fullscreen"`, champ `isSet` |
