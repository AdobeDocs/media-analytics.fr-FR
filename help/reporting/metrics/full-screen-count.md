...
Le serveur principal du média incrémente le champ `count` dans l’entrée `fullscreen` de `mediaReporting.states[]` sur chaque événement de démarrage d’état en plein écran. La mesure est signalée lors de l’appel de fermeture.
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/fr/docs/experience-platform/xdm/data-types/media-reporting-details) l’entrée où `name = "fullscreen"`, champ `count` |
