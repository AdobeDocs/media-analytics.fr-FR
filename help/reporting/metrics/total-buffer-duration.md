...
*Cette page couvre la mesure **Durée totale de la mémoire tampon**. Adobe Analytics renseigne automatiquement une paire [Durée totale de la mémoire tampon (dimension)](/help/reporting/dimensions/total-buffer-duration.md) à partir de la même variable de données contextuelles `a.media.qoe.bufferTime`. Customer Journey Analytics expose un seul champ de `xdm.mediaReporting.qoeDataDetails.bufferTime` que vous pouvez utiliser comme dimension ou mesure.*
