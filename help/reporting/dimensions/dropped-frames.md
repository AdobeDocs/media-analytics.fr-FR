...
*Cette page couvre la dimension **Images perdues**. Adobe Analytics renseigne automatiquement une paire [Images perdues (mesure)](/help/reporting/metrics/dropped-frames.md) à partir de la même variable de données contextuelles `a.media.qoe.droppedFrameCount`. Customer Journey Analytics expose un seul champ de `xdm.mediaReporting.qoeDataDetails.droppedFrames` que vous pouvez utiliser comme dimension ou mesure. Voir [Images perdues](/help/implementation/variables/quality/dropped-frames.md) pour savoir comment collecter cette variable.*
