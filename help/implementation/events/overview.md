...
Une fois qu’une session est ouverte, utilisez [Fin de session](session/session-complete.md) ou [Fin de session](session/session-end.md) pour indiquer comment l’expérience de visionnage s’est terminée. La session d’envoi se termine lorsque le spectateur atteint la fin naturelle du contenu (la vidéo se termine, l’épisode de podcast se termine ou le chapitre final d’un livre audio se termine). La session terminée ne ferme pas la session ; elle reste ouverte jusqu’à son expiration naturelle, de sorte que tous les événements de fin, tels qu’un ping final, sont toujours capturés.
