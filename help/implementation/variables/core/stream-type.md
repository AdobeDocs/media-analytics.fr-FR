...
`streamType` à l’intérieur des `mediaCollection.sessionDetails` lors de l’appel de [`sendEvent`](https://experienceleague.adobe.com/fr/docs/experience-platform/collection/js/commands/sendevent/overview) :
...
`streamType` à l’intérieur des `mediaCollection.sessionDetails` lors de l’appel de `createMediaSession` :
...
Appelez le point d’entrée [sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart) avec `streamType` à l’intérieur du `mediaCollection.sessionDetails` :
