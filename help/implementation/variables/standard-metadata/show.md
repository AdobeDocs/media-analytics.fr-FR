...
| **champ de collection XDM** | [`xdm.mediaCollection.sessionDetails.show`](https://experienceleague.adobe.com/fr/docs/experience-platform/xdm/data-types/session-details-collection) |
| **Caractéristique** | `c_contextdata.a.media.show` |
| **Obligatoire** | Non |
...
`show` à l’intérieur des `xdm.mediaCollection.sessionDetails` lors de l’appel de [`sendEvent`](https://experienceleague.adobe.com/fr/docs/experience-platform/collection/js/commands/sendevent/overview) :
...
Appelez le point d’entrée [sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart) avec `show` à l’intérieur du `xdm.mediaCollection.sessionDetails` :
...
Utilisez `ADBMobile.media.VideoMetadataKeys.SHOW` pour définir le nom de l’émission dans la propriété `StandardMediaMetadata` de l’objet média avant d’appeler `trackSessionStart` :
...
Utilisez `MEDIA_VideoMetadataKeySHOW` pour définir le nom de l’émission dans les métadonnées standard de l’objet média avant d’appeler `mediaTrackSessionStart` :
