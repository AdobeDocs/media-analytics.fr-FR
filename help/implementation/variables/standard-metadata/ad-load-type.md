...
| **champ de collection XDM** | [`xdm.mediaCollection.sessionDetails.adLoad`](https://experienceleague.adobe.com/fr/docs/experience-platform/xdm/data-types/session-details-collection) |
...
`adLoad` à l’intérieur des `xdm.mediaCollection.sessionDetails` lors de l’appel de [`createMediaSession`](https://experienceleague.adobe.com/fr/docs/experience-platform/collection/js/commands/createmediasession) :
...
Utilisez `MEDIA_VideoMetadataKeyAD_LOAD` pour définir le type de chargement de l’annonce publicitaire dans les métadonnées standard de l’objet média avant d’appeler `mediaTrackSessionStart` :
