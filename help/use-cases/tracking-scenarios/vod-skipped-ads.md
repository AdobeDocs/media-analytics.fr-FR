...
Il s’agit du même scénario que [Lecture VOD avec publicités preroll](/help/use-cases/tracking-scenarios/vod-preroll-ads.md), à la différence que l’application permet à l’utilisateur de sauter la publicité, par exemple en cliquant sur un bouton Ignorer.

| Déclencheur   | Méthode Heartbeat  | Appels réseau   | Notes   |
| --- | --- | --- | --- |
| L’utilisateur clique sur [!UICONTROL Lecture]. | `trackSessionStart()` | Analytics Content Start, Heartbeat Content Start | La bibliothèque de mesures ignore l’existence d’une publicité preroll. Ces appels réseau sont exactement les mêmes que la lecture [VOD sans publicité](/help/use-cases/tracking-scenarios/vod-no-intrs-details.md). |
...
| La coupure publicitaire se termine. | `trackEvent:AdBreakComplete` | | Requis même lorsque la publicité a été ignorée. Sans cet événement, les événements de publicité sont ignorés et la durée de la publicité ignorée est comptabilisée comme contenu principal. |
...
Les paramètres sont identiques à ceux du scénario [Lecture VOD avec publicités preroll](/help/use-cases/tracking-scenarios/vod-preroll-ads.md) , sauf qu’il n’existe aucun appel de fin de la publicité. `AdBreakComplete` est toujours nécessaire après l’omission pour fermer la coupure publicitaire.
