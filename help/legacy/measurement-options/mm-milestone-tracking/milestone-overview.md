...
Pour configurer les rapports vidéo pour une implémentation de Milestone, accédez à **[!UICONTROL Admin > Gestionnaire de suites de rapports].** Sélectionnez la suite de rapports, puis choisissez **[!UICONTROL Gestion vidéo > Rapports vidéo]:**
...
| `Media.completeCloseOffsetThreshold` | **Syntaxe :** <br/><br/> `s.Media.completeCloseOffsetThreshold = 1` <br/><br/>Ce seuil vous permet de compter une vue vidéo complète quelques secondes avant la fin réelle de la vidéo.  `Media.completeByCloseOffset` doit être défini sur true pour utiliser ce seuil.<br/><br/>La valeur entière renseignée détermine le décalage en secondes par rapport à la fin de la vidéo pour lequel une vue complète est comptabilisée. Cela vous permet de mesurer des vidéos terminées dans des lecteurs vidéo qui n’enregistrent jamais de décalage équivalant à la durée de la vidéo.  <br/><br/>Par défaut, le seuil est défini sur 1 seconde. |
...
| `Media.track` | **Syntaxe :** <br/><br/> `s.Media.track(mediaName)` <br/><br/>Transmet immédiatement l’état actuel de la vidéo ainsi que les variables `Media.trackVars` et Media.trackEvents que vous avez définies. Cette méthode est utilisée avec `Media.monitor`. <br/><br/>Voir [Mesures supplémentaires à l’aide de Media.monitor.](/help/media-overview.md). <br/><br/>Appelez `Media.open` et `Media.play` sur la vidéo avant d’appeler cette méthode. Cette méthode utilise le paramètre suivant : <ul> <li> **mediaName** : nom de la vidéo. Celui-ci doit correspondre au nom utilisé dans `Media.open`.</li> </ul> Cette méthode constitue le seul moyen d’envoyer des variables supplémentaires pendant la lecture de la vidéo. Elle réinitialise les compteurs d’intervalles, en secondes, et de jalons, en pourcentage, pour éviter la double comptabilisation d’accès de suivi. |
